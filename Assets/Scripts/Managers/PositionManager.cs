...
public class PositionManager : MonoBehaviour {
  ...
  private Vector2? generateRandomEmptyPosition(int depth = 0) {
    if (depth > 10) return null;
    var position = new Vector2(Random.Range(0, size), Random.Range(0, size));
    if (positions.ContainsKey(position)) return generateRandomEmptyPosition(depth += 1);
  ...
  public PositionalObject AddPositional() {
    var pos = generateRandomEmptyPosition();
